...
function Tbiseccion.fx( x: real ): real ;
begin
  result:= power(x,2)-(ln(x))*(power(2.71828183,x)); ;
  //result:= power (x,2) - 1 ;
  end;

procedure Tbiseccion.fx_continua(a_ ,b_:real) ;
begin
  while( a_ = b_ ) do
  begin
    try
...
function Tbiseccion.execute: real;
var xant: real;
    i: integer;
    stop: boolean;
    actualerror: real;
...
function Tbiseccion.displaytable: string;
var i: integer;
    body: string;
    begin
      result:= '<header> <h1>Metodo de la biseccion <br> function power(x,2)-(ln(x))*(power(2.71828183,x)); <br></h1>' +
