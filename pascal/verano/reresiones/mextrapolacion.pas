...
function TMextrapolacion.logaritmica():string;
var
  sumX,sumY,x_p,y_p,A,B,xy,xx,bias,pendiente,sumRY,sumyy,difX,difY,tmp:Real;
  i,j:Integer;
  xi_xp,tmp_datos,xx_yy:t_matrix;
  fun:string;
  parse:TParseMath;
begin
  ...
  SetLength(tmp_datos,cantidad,2);
   for i:=0 to cantidad-1 do begin
    if (datos[i][0]=0) or (datos[i][0]<0) then begin
        tmp_datos[i][0]:=ln(0.001);
        end
    else begin
         tmp_datos[i][0]:=ln(datos[i][0]); end;
    //tmp_datos[i][0]:=ln(datos[i][0]);
    tmp_datos[i][1]:=datos[i][1];
   end;
  //sumatoria de datos
  sumX:=0;sumY:=0;
 ...
 { SetLength(xi_xp,cantidad,2);
  ...
  for i:=0 to cantidad-1 do begin
    difY:=difY+(tmp_datos[i][0]-x_p)*(tmp_datos[i][1]-y_p);
    difX:=difX+power(tmp_datos[i][0]-x_p,2);
    xy:=xy+tmp_datos[i][0]*tmp_datos[i][1];
    xx:=xx+power(tmp_datos[i][0],2);
  end;

  pendiente:=difY/difX;
  bias:=y_p-pendiente*x_p;
  //bias:=y_p-pendiente*x_p;
  B:=(cantidad*xy-sumX*sumY)/(cantidad*xx-power(sumX,2));
  A:=y_p-B*x_p;
  fun:=FloatToStr(A)+'+'+FloatToStr(B)+'*ln(x)';
  //fun:=FloatToStr(bias)+'+'+FloatToStr(pendiente)+'*ln(x)';



  //hallando ral cuadrado HALLANDO COEFICIENTE DE ......
  parse.Expression:=fun;
  sumRY:=0;
  for i:=0 to cantidad-1 do begin
    parse.NewValue('x',datos[i][0]);
 ...
 rLOG:=sqrt(sumRY/sumyy); // aqui le pongo a r
