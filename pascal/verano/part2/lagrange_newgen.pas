...
function TLagrange_NewGen.f(x : Real;s: String):Real;
...
function TLagrange_NewGen.f(x : Real;y : Real;s: String):Real;
var MiParse: TParseMath;
begin
  MiParse:= TParseMath.create();
  MiParse.AddVariable('x',x);
...
function TLagrange_NewGen.Lagrange(serie:TStringGrid; xo:Real;text:TStaticText):Real;
var
  fi,i,j :Integer;
  mul,sum,c,d: Real;
  pln:String;
begin
  ...
  begin
    mul:=1;
      ...
      pln:=pln+'(('+(serie.Cells[i,1])+'/'+FloatToStr(c)+'))';
      c:=1;
      if i<>(fi) then
         pln:=pln+'+';
...
procedure TLagrange_NewGen.NewtonGeneralizado(xo:Real; xi:Real; fx:String; fy:String; error:Real; grill:TStringGrid);
var
